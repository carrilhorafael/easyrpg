...
RSpec.describe "Dice" do
  ...
  describe "roll_and_sum" do
    it "returns an integer" do
      expect(Dice.new("1d6").roll_and_sum).to be_an(Integer)
    end

    it "returns an integer between 1 and the number of sides" do
      expect(Dice.new("1d6").roll_and_sum).to be_between(1, 6)
    end

    it "returns an integer with the same number of elements as the count" do
      expect(Dice.new("3d6").roll_and_sum).to be_between(3, 18)
    end

    it "returns an integer with the modifier added" do
      expect(Dice.new("1d6+1").roll_and_sum).to be_between(2, 7)
    end

    it "returns an integer with the modifier subtracted" do
      expect(Dice.new("1d6-1").roll_and_sum).to be_between(0, 5)
    end
  end
end
